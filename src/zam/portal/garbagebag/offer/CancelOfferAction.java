...
public class CancelOfferAction extends FrameworkAction {

	public CancelOfferAction(HttpServletRequest request, HttpServletResponse response) {
	...
	public RequestDispatcher execute() throws NoConnectionException {
		...
		if (helper.cancelOffer(conn, offer)) {
			UserHelper userHelper = UserHelper.getInstance();
			request.getSession(false).setAttribute("CURRENT_USER", 
					userHelper.getUser(conn, offer.getOfferedBy().getId()));
			dbox.setTitle(MessageBox.SUCCESSFUL);
			dbox.setText("Offer has been canceled.");			
		} else {
			dbox.setTitle(MessageBox.ERROR);
			dbox.setText("An error occured. Unable to cancel offer. "
					+ "Please try again.");						
		}
		try {
			conn.close();

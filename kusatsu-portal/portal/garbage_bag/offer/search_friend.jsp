...
function doSearch() {
	document.form1.action = "<%=getServletContext().getContextPath() %>/GarbageBag/Offer/SearchFriend.do";
	document.form1.submit();
}

function getForm(usrid) {
	document.form2.user_id.value = usrid;
	document.form2.action = "<%=getServletContext().getContextPath() %>/GarbageBag/Offer/GetForm.do";
...
<div id="wrapper">
	<div id="inner_wrapper">
		...
		<div id="content">
			...
			<legend class="input">Search Friend</legend>
...
<%
	for (User user : searchResult) { %>
				<tr>
					<td><%=user.getId() %></td>
					<td><%=user.getName() %></td>
					<td><input type="button" value="Get" onclick="getForm('<%=user.getId() %>')"></td>
				</tr>
<%
} %>
			</table>
